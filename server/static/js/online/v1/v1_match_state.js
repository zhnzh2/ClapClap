(function () {
    function createMatchStateController(options) {
        ...
        async function joinMatchQueue() {
            ...
            const result = await ApiUtils.apiPost("/v1/api/match/join", {
        ...
        async function fetchMatchStatus() {
            ...
            const result = await ApiUtils.apiGet("/v1/api/match/status");
            if (!result.ok) {
        ...
        async function syncMyMatchState() {
            ...
            const result = await ApiUtils.apiGet(
                `/v1/api/match/me?player_token=${encodeURIComponent(playerToken)}`
            );
        ...
        async function cancelMyMatch() {
            ...
            const result = await ApiUtils.apiPost("/v1/api/match/cancel", {
        ...
        async function fetchMyMatchResult() {
            ...
            const result = await ApiUtils.apiGet(
                `/v1/api/match/result?player_token=${encodeURIComponent(playerToken)}`
            );
            ...
            if (data.matched && data.room_id) {
                ...
                window.location.href = `/v1/room/${data.room_id}`;
            }
        }

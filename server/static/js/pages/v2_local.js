...
async function startGame() {
    ...
    var result = await ApiUtils.apiPost("/api/v2/local/reset", {
...
function handleSettlementResult(settlement) {
    ...
    console.log("[v2] settlement action:", action, "| layer:", settlement.current_speed_layer, "| decisions:", (settlement.decision_requests || []).length);
...
async function fetchRoundSummary() {
    ...
    try {
        var result = await ApiUtils.apiGet("/api/v2/local/state");
...
async function submitDecision() {
    ...
    var result = await ApiUtils.apiPost("/api/v2/local/decision", {
        decisions: decisions,
        auto_resolve: v2Settings.autoResolve,
    });
